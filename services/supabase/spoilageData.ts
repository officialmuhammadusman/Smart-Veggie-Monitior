...
class SpoilageDataService {
  ...
  private transformAIFields(reading: any) {
    console.log("🔧 transformAIFields input:", reading);

    if (!reading) {
      console.warn("⚠️ Reading is null/undefined in transformAIFields!");
      return reading;
    }
    ...
    console.log("✅ transformAIFields output:", transformed);
    return transformed;
  }
  ...
  async getAllStatistics() {
    try {
      ...
      console.log("📊 Raw data for statistics:", data);
      ...
      console.log("📊 Enhanced data for statistics:", enhancedData);
  ...
  async getLatestReadingGlobal() {
    try {
      console.log("🔍 Fetching latest reading from Supabase...");
      ...
      if (!data || data.length === 0) {
        console.warn("⚠️ No data in spoilage_data table!");
        return null;
      }

      console.log("✅ Data exists, passing to transformer:", data[0]);

      // Step 1: Call transformer
      const enhanced = SupabaseToAppTransformer.getLatestReading(
        data as SupabaseRawData[],
        this.deviceId,
      );

      console.log("🔧 After transformer (enhanced):", enhanced);

      if (!enhanced) {
        console.error("❌ Transformer returned null!");
        return null;
      }

      // Step 2: Add AI field aliases
      const transformed = this.transformAIFields(enhanced);

      console.log("✅ Final result with ai_ aliases:", transformed);

      return transformed;
    } catch (error) {
      console.error("❌ Error in getLatestReadingGlobal:", error);
      return null;
    }
  }
  ...
  async getChartDataGlobal(days: number = 7) {
    try {
      ...
      console.error("❌ Error in getChartDataGlobal:", error);
      return [];
    }
  }
  ...
  async generateAlertsForFrontend(userId: string) {
    try {
      ...
      console.error("❌ Error in generateAlertsForFrontend:", error);
      return [];
    }
  }
  ...
  async checkCriticalConditions(userId: string) {
    try {
      ...
      console.error("❌ Error in checkCriticalConditions:", error);
      return [];
    }
  }
  ...
  async generateHistoricalTrend(deviceId: string, days: number = 7) {
    try {
      ...
      console.error("❌ Error in generateHistoricalTrend:", error);
      return [];
    }
  }
}
